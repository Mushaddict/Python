...
class Main(QMainWindow, Ui_MainWindow):
    ...
    def show_house_number(self):
        region, percentage = house_analysis.get_house_number()    # 获取房子区域与数量百分比
        chart.pie_chart(percentage,region,'各区二手房数量所占比例') # 显示图表

    # 显示全市二手房装修程度分析
    def show_renovation(self):
        type, number = house_analysis.get_renovation()             # 获取全市房子装修程度
        chart.renovation_bar(type,number,'全市二手房装修程度分析')       # 显示图表
